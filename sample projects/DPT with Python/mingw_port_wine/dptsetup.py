...
Run DPT API build using Mingw under Wine on FreeBSD.
...
if __name__ == '__main__':
    ...
    def MakeTarget(target, defines, options, settings):
        ...
        job = ['gmake', target]
        for s in settings:
            if s.startswith('PATH_TO_CXX='):
                break
        else:
            job.append('PATH_TO_CXX=C:/mingw/bin/')
        for s in settings:
            if s.startswith('POSIX_PATH_TO_PYTHON='):
                break
        else:
            job.append('POSIX_PATH_TO_PYTHON=~/.wine/drive_c/')
        for s in settings:
            job.append(s)
        job.extend([
            ''.join(('DPT_NAMES=',
